...
When creating Sessions optional nifcloud.Config values can be passed in that will
override the default, or loaded config values the Session is being created
with. This allows you to provide additional, or case based, configuration
as needed.

By default NewSession will only load credentials from the shared credentials
file (~/.nifcloud/credentials). If the AWS_SDK_LOAD_CONFIG environment variable is
set to a truthy value the Session will be created from the configuration
values from the shared config (~/.nifcloud/config) and shared credentials
(~/.nifcloud/credentials) files. See the section Sessions from Shared Config for
more information.
...
By default the SDK will only load the shared credentials file's (~/.nifcloud/credentials)
credentials values, and all other config is provided by the environment variables,
SDK defaults, and user provided nifcloud.Config values.

If the AWS_SDK_LOAD_CONFIG environment variable is set, or SharedConfigEnable
option is used to create the Session the full shared config values will be
loaded. This includes credentials, region, and support for assume role. In
addition the Session will load its configuration from both the shared config
file (~/.nifcloud/config) and shared credentials file (~/.nifcloud/credentials). Both
files have the same format.

If both config files are present the configuration from both files will be
read. The Session will be created from configuration values from the shared
credentials file (~/.nifcloud/credentials) over those in the shared config file (~/.nifcloud/config).
...
(~/.nifcloud/config) or shared credentials (~/.nifcloud/credentials) file.
